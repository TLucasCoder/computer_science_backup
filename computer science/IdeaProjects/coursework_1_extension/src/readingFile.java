        ...
        try{
            ...
            while (sc.hasNextLine()){
                ...
                if (count <=3){
                    ...
                    String [] speed_note = null;
                    temp_read = temp_read.substring(1,temp_read.length()-1);
                    speed_note = temp_read.split(",");
                    for (int i = 0; i< speed_note.length; i++){
                        speed_note[i] = speed_note[i].trim();
                    }
                    temp_compo.get(temp_compo.size() - 1).addSpeed(speed_note);
                }
                else{
                    String ins_type = "";
