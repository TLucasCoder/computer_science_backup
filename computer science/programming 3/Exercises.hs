...
--formCNF  store (Or (Not a:as)) = And [ Or (store++[Not a])]
--formCNF  store (Or (Var a:as)) = And [ Or (store++[Var a])]
...
formCNF store (And l@(a:as)) 
        ...
        | otherwise =  And $  formCNF store (And [a]) : [formCNF store (And as)] 
-- : [formCNF store (And as)]

formCNF  store l@(Not a) = And [ Or (store++[l])]
formCNF  store l@(Var a) = And [ Or (store++[l])]


formString :: Expr -> String
formString exep = read (show exep)
